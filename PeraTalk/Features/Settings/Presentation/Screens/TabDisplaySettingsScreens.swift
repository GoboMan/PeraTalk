...
private enum ScreenDisplaySettingsViewBindings {
    ...
    static func vocabularyShowPartOfSpeech(_ model: ScreenDisplaySettingsScreenModel) -> Binding<Bool> {
        Binding(
            get: { model.preferences.vocabularyList.showPartOfSpeech },
            set: { newValue in
                Task { await model.update { $0.vocabularyList.showPartOfSpeech = newValue } }
            }
        )
    }
        ...
        Group {
            if let model {
                Form {
                    ...
                    Section {
                        Toggle("品詞を表示", isOn: ScreenDisplaySettingsViewBindings.vocabularyShowPartOfSpeech(model))
                        ...
                        Text("見出し語は常に表示されます。それ以外をスイッチで切り替えられます。発音は用法に IPA などが登録されているときに表示されます。")
                    }
                }
            } else {
                ProgressView()

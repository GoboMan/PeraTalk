...
| `lemmas` | 配列。要素は `stable_lemma_id`（UUID）, `lemma`, `language?`, **`usages[]`** |
| `usages[]` | `kind`（`VocabularyKind.rawValue`）, `position`, 任意で `definition_target`, `definition_aux`, `ipa`, 任意 **`study_headword`**（用法別の代表的英語綴り。**省略時はサーバー側で導出しない**—インポート時にクライアントが `surfaces` 規則で埋める）、**`surfaces[]`** |
| `surfaces[]`（各用法の子） | `text`, `form_kind`（`LemmaSurfaceFormKind` と同値。当該 `kind` と整合するもののみ）, `ipa?` |
...
| `study_headword` | `text` |  |  | 例文などに載せる**用法別の代表的英語綴り**（親単語ヘッドとの差異の例 allocate / allocated）。省略時や NULL は読み側で親 `vocabulary` のヘッド見出しへフォールバックしてよい |

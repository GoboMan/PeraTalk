...
- **キャッシュとローカル正データの区別**：エンティティ名で**接頭辞 `Cached` を付ける**ことで「**サーバーが正（クロスデバイスで揃える対象）**」を表す。**双方向同期（端末発生 → push、別端末で pull）も含まれる**点に注意。「クライアントで生成 → サーバーに push」の流れでも、**他端末から見たときの正はサーバー**であり、その意味で `Cached*` を一貫して用いる。**例外**：**プリセット英語辞典**（`CachedLemma` / `CachedLemmaSurface` / `CachedDictionaryPackMeta`）は **Supabase のユーザー行ではなく**、**端末内の読み取り専用マスタ**として `Cached` 接頭辞を流用している（[§3.7.1](#371-プリセット英語辞典端末ローカル同期対象外)）。**Phase 1 で `Cached*` 化されない端末ローカル限定エンティティは存在しない**（Phase 1 の `CachedUtterance` は **サーバーが正だが Phase 1 では同期しない例外**として扱う。詳細は §1.3）。
...
- **端末ローカル専用（読み取り専用マスタ・push/pull 対象外）**：`CachedLemma` / `CachedLemmaSurface` / `CachedDictionaryPackMeta`（辞書パック適用メタ）
...
- **同梱インポート**（開発〜本格化の増分）: **`dictionary_scaffold_pack.json` が単一ソース（SSOT）**。インポータはこれのみを適用する。`dictionary_sample_pack.json` はスキーマ例の参照用。
...
- **分詞形容詞を動詞レマに併記**：`allocate` のように `verb_*` と `adj_*` を**同一 `CachedLemma`** に載せ、`posRaw` は主として `verb` とする場合、**別 `stable_lemma_id` を増やさない**（[単語帳 §1.1](../機能/単語帳.md#11-kind品詞一覧)）。UI は動詞表・形容詞表の二ブロックを同一レマから出し分ける。
...
| `posRaw` | `String` | ✓ | `VocabularyKind` の rawValue と整合する想定 |
| `languageCode` | `String` | ✓ | 例: `en` |
| `definitionTarget` | `String?` |  | 英語など**ターゲット言語**の短い定義（辞書パックの `definition_target`）。[単語帳](../機能/単語帳.md) の「定義（ターゲット）」のプリセット元。 |
| `definitionAux` | `String?` |  | **補助言語**の短い定義（パックの `definition_aux`）。 |

- **関係**：`surfaces: [CachedLemmaSurface]`（`.cascade`）／`linkedVocabularyEntries: [CachedVocabulary]`（inverse: `CachedVocabulary.lemma`。**nullify**）。
